    ...
    <>
      ...
      <section className={`content ${styles.content}`}>
        <header className={styles.header}>
          <div>
            <p className={styles.eyebrow}>CAPACITY / TREND / FORECAST</p>
            <h1>Capacity &amp; Trend Forecast</h1>
            <p>
              Host Disk・Memory、Container Memory、Backupサイズの実測履歴から増加傾向を評価し、
              データ品質を満たす場合だけしきい値到達の目安を表示します。
            </p>
          </div>
          <div className={styles.actions}>
            <a href="/inventory">Inventory</a>
            <a href={`/history?range=${range}`}>History</a>
        ...
        {loadError || !data ? (
          <div className="empty error-panel" role="alert">
            <strong>Capacity情報を取得できませんでした</strong>
            <p>Monitoring / History / BackupのServer-side接続を確認してください。</p>
          </div>
        ) : (
          <>
            {sourceFailures.length > 0 ? (
              <div className={styles.coverage} role="status">
                一部データソースを取得できないため、取得できた情報だけで継続表示しています。
                <span>{sourceFailures.join(" / ")}</span>
              </div>
            ) : null}

            <section className={styles.summary} aria-label="Capacity Forecast Summary">
              <article className={styles.summaryPrimary}>
                <span>FORECAST ATTENTION</span>
                <strong>{data.summary.forecastAttentionCount}</strong>
                <small>現在値または予測しきい値に注意が必要</small>
              </article>
              <article><span>CRITICAL</span><strong>{data.summary.criticalCount}</strong><small>現在 / 7日以内予測</small></article>
              <article><span>WARNING</span><strong>{data.summary.warningCount}</strong><small>現在 / 30日以内予測</small></article>
              <article><span>GROWTH</span><strong>{data.summary.growthCount}</strong><small>増加傾向のみ検出</small></article>
              <article><span>INSUFFICIENT</span><strong>{data.summary.insufficientCount}</strong><small>予測品質ゲート未達</small></article>
              <article><span>BACKUP GROWTH</span><strong>{data.summary.backupGrowthCount}</strong><small>容量上限は仮定しません</small></article>
            </section>
            ...
            <section className={styles.section} aria-labelledby="host-capacity-title">
              ...
              {data.hostResources.length === 0 ? <div className={styles.empty}><strong>Host Capacityデータがありません</strong><p>Host HistoryまたはHeartbeat受信後に表示されます。</p></div> : null}
            </section>
            ...
            <section className={styles.section} aria-labelledby="container-capacity-title">
              ...
              {data.containerResources.length === 0 ? <div className={styles.empty}><strong>Container Capacityデータがありません</strong><p>Container History受信後に表示されます。</p></div> : null}
            </section>
            ...
            <section className={styles.section} aria-labelledby="backup-growth-title">
              ...
              {data.backupGrowth.length === 0 ? <div className={styles.empty}><strong>Backupサイズ履歴がありません</strong><p>ReporterからsizeBytesを含む成功Runが蓄積されると傾向を表示します。</p></div> : null}
            </section>
