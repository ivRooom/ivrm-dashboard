...
const coverage = (complete: boolean) => complete ? "Complete coverage" : "Partial coverage";
...
export function InventorySummary({ data }: { data: InfrastructureInventory }) {
  return (
    <section className={styles.summaryGrid} aria-label="Inventoryサマリー">
      ...
      <article><span>CPU CAPACITY</span><strong>{data.summary.cpuCoreCount ?? "不明"}</strong><small>{coverage(data.summary.cpuCapacityComplete)}</small></article>
      <article><span>MEMORY / DISK</span><strong>{bytes(data.summary.memoryTotalBytes)}</strong><small>Disk {bytes(data.summary.diskTotalBytes)} / Memory {coverage(data.summary.memoryCapacityComplete)} / Disk {coverage(data.summary.diskCapacityComplete)}</small></article>
    </section>
  );
}
