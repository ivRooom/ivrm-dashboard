revoke update on table public.reliability_slo_policies from service_role;

create or replace function public.update_reliability_slo_policy_v1(
  p_service_id text,
...
grant execute on function public.update_reliability_slo_policy_v1(
  text,
  numeric,
  boolean,
  uuid,
...
comment on function public.update_reliability_slo_policy_v1(
  text,
  numeric,
  boolean,
  uuid,
  ...
  'Atomically updates one explicit Reliability SLO policy and appends a hash-chained audit log entry.';
