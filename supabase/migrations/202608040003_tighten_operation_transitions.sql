...
begin
  ...
  elsif p_expected_status = 'leased' and p_new_status = 'queued' then
    if p_actor_type not in ('agent', 'system') then
      raise exception 'operation_transition_actor_denied'
        using errcode = '42501';
    end if;
  elsif p_expected_status = 'leased' and p_new_status = 'failed' then
    if p_actor_type not in ('agent', 'system') then
      raise exception 'operation_transition_actor_denied'
        using errcode = '42501';
    end if;
  ...
  elsif p_expected_status = 'running'
    and p_new_status in ('succeeded', 'failed') then
    if p_actor_type <> 'agent' then
      raise exception 'operation_transition_actor_denied'
        using errcode = '42501';
    end if;
